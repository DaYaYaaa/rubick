local rubick = {}
rubick.spellkey, rubick.swapkey =
    Menu.AddKeyOption({".SCRIPTS", "Rubick"}, "Rubick Spell Key", Enum.ButtonCode.KEY_SPACE),
    Menu.AddKeyOption({".SCRIPTS", "Rubick"}, "Rubick Swap Key", Enum.ButtonCode.KEY_X)

function rubick.OnUpdate()
    local self = Heroes.GetLocal()
    if self and NPC.GetUnitName(self) ~= "npc_dota_hero_rubick" then
        return
    end
    local telekinesis, fadebolt, spell, spellsteal =
        NPC.GetAbilityByIndex(self, 0),
        NPC.GetAbilityByIndex(self, 1),
        NPC.GetAbilityByIndex(self, 3),
        NPC.GetAbilityByIndex(self, 5)
    if
        NPC.GetAbility(self, "special_bonus_cast_range_125") and
            Ability.GetLevel(NPC.GetAbility(self, "special_bonus_cast_range_125")) > 0
     then
    ...
    for enemyindex = 1, Heroes.Count() do
        ...
        if
            enemy and not Entity.IsSameTeam(self, enemy) and not NPC.IsChannellingAbility(self) and
                not NPC.HasState(self, Enum.ModifierState.MODIFIER_STATE_INVISIBLE)
         then
            for abilityindex = 0, 24 do
                ...
                if enemyspell and Ability.GetCooldown(enemyspell) > 0 then
                    if spellsteal and Ability.IsReady(spellsteal) and NPC.IsEntityInRange(self, enemy, range) then
                        if
                            spell and Ability.GetCooldown(spell) > 0 or NPC.GetAbility(self, "rubick_empty1") or
                                Menu.IsKeyDown(rubick.swapkey)
                         then
                            if
                                spell and Ability.GetName(spell) ~= Ability.GetName(enemyspell) and
                                    (not rubick[Ability.GetName(enemyspell)] or
                                        GameRules.GetGameTime() - rubick[Ability.GetName(enemyspell)] >
                                            Ability.GetCooldownLength(enemyspell))
                             then
                                Ability.CastTarget(spellsteal, enemy)
                                rubick[Ability.GetName(enemyspell)] = GameRules.GetGameTime()
                            end
                        end
                    end
                end
    ...
    if spell then
        if
            (Ability.GetBehavior(spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_CHANNELLED) ~= 0 and
                Ability.IsReady(spell) or
                NPC.IsChannellingAbility(self)
         then
            ischannelling = true
        else
            ischannelling = false
        ...
        for _, library in pairs(
            ...
            if library then
                if Ability.GetName(spell) == library.name then
                    if library.type == "evade" then
                    end
                    if library.type == "disable" then
                        isdiable = true
                    else
                        isdiable = false
                    end
                    if distance == 0 or distance == 125 then
                        distance = library.radius - 75
                    end
                end
            end
        end
        ...
        if
            Ability.GetName(spell) == "nevermore_shadowraze1" or Ability.GetName(spell) == "nevermore_shadowraze2" or
                Ability.GetName(spell) == "nevermore_shadowraze3"
         then
            distance = distance + 150
        end
        if enemy and Menu.IsKeyDown(rubick.spellkey) then
            if
                Entity.GetAbsOrigin(self):Distance(Entity.GetAbsOrigin(enemy)):Length2D() < distance and
                    Ability.IsReady(spell)
             then
                if
                    not isdiable or
                        isdiable and not NPC.IsStunned(enemy) and
                            not NPC.HasState(enemy, Enum.ModifierState.MODIFIER_STATE_HEXED)
                 then
                    if (Ability.GetBehavior(spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0 then
                        if
                            Ability.GetName(spell) == "earthshaker_enchant_totem" and
                                NPC.GetItem(self, "item_ultimate_scepter")
                         then
                            Ability.CastPosition(spell, Entity.GetAbsOrigin(enemy))
                        elseif
                            Ability.GetName(spell) == "riki_tricks_of_the_trade" and
                                NPC.GetItem(self, "item_ultimate_scepter")
                         then
                            Ability.CastTarget(spell, self)
                        elseif
                            Ability.GetName(spell) == "chaos_knight_phantasm" and
                                NPC.GetItem(self, "item_ultimate_scepter")
                         then
                            Ability.CastTarget(spell, self)
                        elseif Ability.GetName(spell) == "luna_eclipse" and NPC.GetItem(self, "item_ultimate_scepter") then
                            Ability.CastPosition(spell, Entity.GetAbsOrigin(enemy))
                        elseif
                            Ability.GetName(spell) == "nevermore_shadowraze1" or
                                Ability.GetName(spell) == "nevermore_shadowraze2" or
                                Ability.GetName(spell) == "nevermore_shadowraze3" or
                                Ability.GetName(spell) == "slark_pounce"
                         then
                            if
                                Ability.GetName(spell) == "nevermore_shadowraze1" or
                                    Ability.GetName(spell) == "slark_pounce" or
                                    Ability.GetName(spell) == "nevermore_shadowraze2" and
                                        Entity.GetAbsOrigin(self):Distance(Entity.GetAbsOrigin(enemy)):Length2D() > 325 or
                                    Ability.GetName(spell) == "nevermore_shadowraze3" and
                                        Entity.GetAbsOrigin(self):Distance(Entity.GetAbsOrigin(enemy)):Length2D() > 575
                             then
                    ...
                    if (Ability.GetBehavior(spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) ~= 0 then
                        ...
                        if (Ability.GetTargetTeam(spell) & Enum.TargetTeam.DOTA_UNIT_TARGET_TEAM_ENEMY) ~= 0 then
                            if
                                not NPC.HasModifier(enemy, "modifier_antimage_counterspell") and
                                    not NPC.HasModifier(enemy, "modifier_item_lotus_orb_active")
                             then
                                Ability.CastTarget(spell, enemy)
                            end
                        else
                    ...
                    if (Ability.GetBehavior(spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_POINT) ~= 0 then
                        if Ability.GetName(spell) == "pugna_nether_ward" then
                            ...
                            if Ability.GetName(spell) == "pudge_meat_hook" then
                                for i = 1, math.floor(
                                    (Entity.GetAbsOrigin(self) - Entity.GetAbsOrigin(enemy)):Length2D() / 125
                                ) do
                                    local isblocked = false
                                    for _, unit in ipairs(
                                        NPCs.InRadius(
                                            Entity.GetAbsOrigin(self) +
                                                (Entity.GetAbsOrigin(enemy) - Entity.GetAbsOrigin(self)):Normalized():Scaled(
                                                    i * 125
                                                ),
                                            125,
                                            Entity.GetTeamNum(self),
                                            Enum.TeamType.TEAM_BOTH
                                        )
                                    ) do
                                        if
                                            unit and Entity.IsNPC(unit) and unit ~= enemy and unit ~= self and
                            ...
                            if
                                not isblocked and Ability.GetName(spell) == "pudge_meat_hook" and
                                    NPC.FindFacingNPC(self, nil, Enum.TeamType.TEAM_ENEMY, nil, 15) == enemy or
                                    Ability.GetName(spell) ~= "pudge_meat_hook"
                             then
                            ...
                            if
                                Ability.GetName(spell) == "ancient_apparition_ice_blast" and
                                    Ability.IsInAbilityPhase(spell)
                             then
                                timer = GameRules.GetGameTime()
                                traveltime =
                                    Entity.GetAbsOrigin(self):Distance(Entity.GetAbsOrigin(enemy)):Length2D() / 1500
                            end
                        end
                    end
                end
            ...
            if not spell or spell and not ischannelling then
                if not NPC.IsStunned(enemy) and not NPC.HasState(enemy, Enum.ModifierState.MODIFIER_STATE_HEXED) then
                    if telekinesis and Ability.IsReady(telekinesis) then
                        if
                            not spell or not isdiable or
                                spell and isdiable and
                                    Ability.GetCooldownLength(spell) - Ability.GetCooldownTimeLeft(spell) > 1 or
                                spell and isdiable and distance > 0 and
                                    Entity.GetAbsOrigin(self):Distance(Entity.GetAbsOrigin(enemy)):Length2D() > distance
                         then
